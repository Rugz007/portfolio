const Technologies = [
  {
    title: "Modern Web",
    iconLink: [
      <img
        alt="nestjs"
        style={{ width: "48px" }}
        src="https://docs.nestjs.com/assets/logo-small.svg"
      />,
      <img alt="go" src="https://img.icons8.com/color/48/000000/golang.png" />,
      <img
        alt="graphql"
        src="https://img.icons8.com/color/48/000000/graphql.png"
      />,
      <img alt="nextjs" src="next.png" />,
    ],
    description:
      "Building robust, scalable and fast web applications with cutting edge technologies like NestJS, GraphQL, NextJS, Go, etc.",
  },

  {
    title: "React and Redux",
    iconLink: [
      <img
        alt="react"
        style={{ margin: "0% 2%" }}
        src="https://img.icons8.com/color/48/000000/react-native.png"
      />,
      <img
        alt="redux"
        style={{ margin: "0% 2%" }}
        src="https://img.icons8.com/color/48/000000/redux.png"
      />,
    ],
    description:
      "Developing robust and responsive frontend with React with Context or Redux, with proficiency in using variety UI libraries.",
  },

  {
    title: "Python & Django",
    iconLink: [
      <img
        alt="python"
        style={{ margin: "0% 2%" }}
        src="https://img.icons8.com/color/50/000000/python.png"
      />,
      <img
        alt="django"
        style={{ width: "54px" }}
        src="data:image/png;base64,"
      />,
      <img
        alt="rabitmq"
        style={{ width: "40px", marginLeft: "3%" }}
        src="https://png2.cleanpng.com/sh/970efdece240fb71aa28c5dfc130afb9/L0KzQYm3WMMxN6R4eZH0aYP2gLBuTgJiapNujN96LYPmcb3ogvxmNadqeAZ4cj3qgrL3iPlke15ohNt5LXH1hH73jCJ1aZNxReRqYnLshL74Tfxwb5CyiAhwLYboc8X2kr1idaEyiNDwLYT1cbB6kPFzbZ95Reg2NXK7SIPtWPVlQGRqfKQ3M0O6RoGBUsEyPWQ6Tqc6N0K5SIm8WL5xdpg=/kisspng-rabbitmq-scalable-vector-graphics-clip-art-portabl-rabbitmq-logo-svg-vector-amp-png-transparent-v-5b882f8ed83ed2.3376082115356517268858.png"
      />,
      <img
        alt="redis"
        src="https://img.icons8.com/color/48/000000/redis.png"
      />,
    ],
    description:
      "Making robust backend using Django with DjangoRESTFramework equipped with Redis or RabbitMQ",
  },
  {
    title: "Web-Dev Essentials",
    iconLink: [
      <img
        alt="html"
        src="https://img.icons8.com/color/50/000000/html-5.png"
      />,
      <img alt="css" src="https://img.icons8.com/color/48/000000/css3.png" />,
      <img
        alt="bootstrap"
        src="https://img.icons8.com/color/48/000000/bootstrap.png"
      />,
      <img
        alt="less"
        src="data:image/png;base64,"
      />,
    ],
    description: "Making websites using HTML5, CSS3,Bootstrap and less.",
  },
  {
    title: "Cloud and DevOps",
    iconLink: [
      <img
        alt="docker"
        src="https://img.icons8.com/color/48/000000/docker.png"
      />,
      <img
        alt="kubernetes"
        src="https://img.icons8.com/color/50/000000/kubernetes.png"
      />,
      <img
        alt="aws"
        src="https://img.icons8.com/color/48/000000/amazon-web-services.png"
      />,
    ],
    description:
      "Using Docker and Kubernetes while deploying on cloud services like AWS and Digital Ocean",
  },
  {
    title: "Databases",
    iconLink: [
      <img
        alt="mysql"
        style={{ width: "48px" }}
        src="https://cdn.iconscout.com/icon/free/png-512/mysql-19-1174939.png"
      />,
      <img
        alt="postgres"
        src="https://img.icons8.com/color/48/000000/postgreesql.png"
      />,
      <img
        alt="firebase"
        src="https://img.icons8.com/color/48/000000/google-firebase-console.png"
      />,
    ],
    description: "Knowledge of using MySQL,PostgresSQL and Firebase.",
  },
  {
    title: "Machine and Deep Learning",
    iconLink: [
      <img
        alt="tensorflow"
        src="https://img.icons8.com/color/48/000000/tensorflow.png"
      />,
      <img
        style={{ width: "48px" }}
        alt="streamlit"
        src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/05/Scikit_learn_logo_small.svg/1200px-Scikit_learn_logo_small.svg.png"
      />,
      <img
        style={{ width: "48px" }}
        alt="streamlit"
        src="https://avatars3.githubusercontent.com/u/45109972?s=400&v=4"
      />,
    ],
    description:
      "Proficient in using Tensorflow and scikit-learn with Streamlit.",
  },
  {
    title: "Game Development",
    iconLink: [
      <img alt="unity" src="https://img.icons8.com/nolan/64/unity.png" />,
      <img
        alt="c-sharp"
        src="https://img.icons8.com/color/48/000000/c-sharp-logo-2.png"
      />,
    ],
    description:
      "Can make quality games in Unity game engine with proficiency in C#.",
  },
  {
    title: "Programming Languages",
    iconLink: [
      <img
        alt="java"
        src="https://img.icons8.com/color/48/000000/java-coffee-cup-logo.png"
      />,
      <img
        alt="c++"
        src="https://img.icons8.com/color/48/000000/c-plus-plus-logo.png"
      />,
      <img
        alt="javascript"
        src="https://img.icons8.com/color/48/000000/javascript.png"
      />,
      <img
        alt="typescript"
        src="https://img.icons8.com/color/48/000000/typescript.png"
      />,
    ],
    description:
      "Proficiency in languages like Java,C++,JavaScript and TypeScript.",
  },

  {
    title: "",
    iconLink: [],
    description: "",
  },
  {
    title: "Tools",
    iconLink: [
      <img alt="git" src="https://img.icons8.com/color/48/000000/git.png" />,
      <img
        alt="figma"
        style={{ width: "48px" }}
        src="https://i.pinimg.com/originals/a5/58/b4/a558b426cb8973523f37bbed94cf0f09.png"
      />,
      <img
        alt="photshop"
        src="https://img.icons8.com/color/48/000000/adobe-photoshop.png"
      />,
      <img
        alt="davinci"
        src="https://img.icons8.com/color/48/000000/davinci-resolve.png"
      />,
      <img
        alt="blender"
        src="https://img.icons8.com/color/48/000000/blender-3d.png"
      />,
    ],
    description:
      "Able to use modern tools like Git,Figma,Photoshop ,DaVinci Resolve & Blender.",
  },
  {
    title: "",
    iconLink: [],
    description: "",
  },
];

export default Technologies;
